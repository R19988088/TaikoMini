...
class GameRenderer:
    ...
    def reset(self):
        ...
        self.position_cache.clear()  # 清空位置缓存
    ...
    def draw_bars(self, bars, game_time):
        ...
        if cache_key in self.position_cache:
            ...
            for bar in bars:
                ...
                bars_to_draw.append(int(x))
            
            # 缓存结果
            self.position_cache[cache_key] = bars_to_draw
        
        # 批量绘制所有小节线（减少draw调用）
        for x in bars_to_draw:
            pygame.draw.line(
                self.screen,
                WHITE,
    ...
    def draw_notes(self, notes, current_note_index, game_time, combo=0):
        """
        ...
        """
        # 检查是否需要重新计算位置（时间变化时）
        cache_key = f"notes_{current_note_index}_{int(game_time // 16)}"  # 每16ms缓存一次
        if cache_key in self.position_cache and abs(game_time - self.last_game_time) < 16:
            # 使用缓存的位置数据
            notes_to_draw = self.position_cache[cache_key]
        else:
            # 重新计算位置
            notes_to_draw = []
            ...
            # 缓存计算结果
            self.position_cache[cache_key] = notes_to_draw
    ...
    def _draw_drumroll_with_texture(self, start_x, end_x, y, body_texture, front_texture, tail_texture, note_type, game_time):
        ...
        if note_type in [7, 9]:
            ...
            # 气球连打到达判定线后停住
            # 气球图案宽度204，普通音符260，差值56像素
            # 整体向左偏移28像素，使笑脸音符中心对齐判定圈中心
            balloon_offset = -12
            
            # 如果start_x（移动中的位置）已经到达或经过判定线，则固定在判定线
            display_x = start_x + balloon_offset
            if start_x <= self.judge_x:
                display_x = self.judge_x + balloon_offset
        ...
        # 1. 绘制前端图案（中心对齐在start_x）
        front_rect = front_texture.get_rect(center=(start_x, y))
        self.screen.blit(front_texture, front_rect)
        
        # 2. 计算主体绘制区域（从前端图案右边缘到尾端图案左边缘）
        body_start_x = start_x + front_width // 2
    ...
    def draw_drumrolls(self, notes, game_time, combo=0):
        """
        ...
        if not hasattr(self, '_drumroll_pairs'):
            self._drumroll_pairs = []
                ...
                if note.type in [5, 6, 7, 9]:
                    for j in range(i + 1, len(notes)):
                        if notes[j].type == 8:
                            ...
                            self._drumroll_pairs.append((note, notes[j], original_type))
                            break
        
        # 批量收集需要绘制的连打条
        drumrolls_to_draw = []
        
        for start_note, end_note, original_type in self._drumroll_pairs:
