...
class SongInfoDisplay:
    ...
    def __init__(self, screen, config_manager=None):
        ...
        # 分类图片目录 - 使用Android适配器获取正确路径
        if _adapter and _adapter.is_android:
            songs_folder = _adapter.get_songs_folder()
            self.genre_image_dir = songs_folder / "Resource"
        else:
            self.genre_image_dir = Path("songs/Resource")
    ...
    def _load_fonts(self):
        ...
        # === 分类字体：优先使用系统黑体 ===
        category_fonts = ['SimHei', 'Microsoft YaHei', 'MS Gothic', 'Meiryo', 'Arial']
        for font_name in category_fonts:
            try:
                if pygame.font.match_font(font_name):
                    self.font_category = pygame.font.SysFont(font_name, self.category_font_size)
                    print(f"Loaded category font: {font_name} (size: {self.category_font_size})")
                    return
            except:
                continue
