...
from lib.utils import get_pixels_per_frame, global_data, strip_comments
...
class Note:
    ...
    time_signature: float = field(init=False, default=1.0)  # 拍号（如4/4=1.0, 3/4=0.75）
    is_branch_start: bool = field(init=False)
    branch_params: str = field(init=False)
    is_super_large: bool = field(init=False)  # J/K/L/M 超大音符标记
    
    def __post_init__(self):
        """初始化字段默认值"""
        if not hasattr(self, 'is_super_large'):
            self.is_super_large = False
...
def calculate_base_score(notes: NoteList) -> int:
    ...
    if total_notes == 0:
        return 1000000
    return math.ceil((1000000 - (balloon_count * 100) - (16.5 * drumroll_msec / 1000 * 100)) / total_notes / 10) * 10
        ...
        for bar in notes:
            ...
            for part in bar:
                if part.startswith('#BRANCHSTART'):
                    ...
                    branch_params = part[13:].strip()  # 去掉前后空格

                    if branch_params and branch_params[0] == 'r':
                ...
                if '#JPOSSCROLL' in part:
                    ...
                    gogo_time = False
                    continue
                #Unrecognized commands will be skipped for now (but allow J/K/L/M)
                elif len(part) > 0 and not part[0].isdigit() and part[0].upper() not in 'JKLM':
                    continue
                ...
                bar_line.bpm = bpm
                bar_line.time_signature = time_signature  # 保存拍号
                if barline_added:
                ...
                for item in part:
                    ...
                    if item.upper() == 'J':
                        ...
                        is_super_large = True
                        note_type = 4  # 大ka
                    elif item == '0' or (not item.isdigit()):
                    ...
                    note.type = note_type
                    note.is_super_large = is_super_large
                    note.index = index
                    note.bpm = bpm
